...
(async function init() { // This is called when code reaches this point
    ...
    encoderModel = await tf.loadLayersModel('https://mateocamara.com/intellimixer/models/encoder_model/model.json');
    logInfo("Loaded default encoder.")
    // // get decoder tensorflow model
    decoderModel = await tf.loadLayersModel('https://mateocamara.com/intellimixer/models/decoder_model/model.json');
...
function setMapDescriptor(init) {
    let x_descriptor = document.getElementById('x_axis_map_descriptors_selector').value - 1;
    let y_descriptor = document.getElementById('y_axis_map_descriptors_selector').value - 1;
    if (init) {
        y_descriptor = 1;
    }
